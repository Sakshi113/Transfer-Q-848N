...
class Collaborative_TQ_indirect:
    def __init__(self, llm_path, rm_path, llm_dev="cuda:0", rm_dev="cuda:1", torch_dtype=torch.float16):
        ...
        self.LLM = AutoModelForCausalLM.from_pretrained("Salesforce/LLaMA-3-8B-SFR-Iterative-DPO-R", torch_dtype=torch_dtype).to('cuda:0') # https://huggingface.co/Salesforce/LLaMA-3-8B-SFR-Iterative-DPO-R
        self.LLM.eval()
        
        #self.tokenizer = AutoTokenizer.from_pretrained("lomahony/eleuther-pythia6.9b-hh-dpo")
        self.tokenizer = AutoTokenizer.from_pretrained("Salesforce/LLaMA-3-8B-SFR-Iterative-DPO-R")
        print("Loading RM...")
        
        # self.RM_1 = AutoModelForSequenceClassification.from_pretrained("usvsnsp/pythia-6.9b-rm-full-hh-rlhf", num_labels=1, torch_dtype=torch_dtype).to('cuda:0')
        self.RM_1 = AutoModelForSequenceClassification.from_pretrained("Salesforce/LLaMA-3-8B-SFR-RM-R", num_labels=1, torch_dtype=torch_dtype).to('cuda:1')
        ...
        self.reward_tokenizer_1 = AutoTokenizer.from_pretrained("Salesforce/LLaMA-3-8B-SFR-RM-R")
...
class Orchestrator:
    ...
    def decide(self, user_prompt, worker_res, critic_res, threshold=5.0):
        ...
        input_ids = self.base_tokenizer(orch_prompt, return_tensors="pt").input_ids.to("cuda:0")
